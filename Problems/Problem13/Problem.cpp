...
Problem13::Problem13()
{
    this->ProblemName = new std::string("Large sum");
    this->ProblemNumber = 13;
}

void Problem13::Execute()
{
    uint64_t carry;
    uint32_t sum;
    uint32_t digit;
    ...
    uint32_t numDigits = (this->numDigits > 49 ? 49 : this->numDigits);
    
    for(int8_t col=numDigits;col>=0;col--) {
        sum = 0;
        digit = 0;
    ...
    while(carry > 0)
    {
        this->AddDigit(carry % 10);
        carry /= 10;
    }
}

void Problem13::GetAnswer()
{
    printf("Answer: ");
    
    while(this->numbers.size() > 0) {
        printf("%u", this->numbers.front());
...
void Problem13::GetInputs()
{
    this->ProjectFunctions->GetInput(&this->numDigits, 10, "number of digits");
}

void Problem13::AddDigit(uint8_t digit)
{
    if(this->numbers.size() >= this->numDigits) {
        this->numbers.pop_back();
    }
    
    this->numbers.push_front(digit);
}